...
/*
 * There are three scenarios, in which AllenModule can be loaded by NEST:
 *
...
allennest::AllenModule allenmodule_LTX_mod;
#endif
// -- DynModule functions ------------------------------------------------------

allennest::AllenModule::AllenModule()
{
#ifdef LINKED_MODULE
...
const std::string
allennest::AllenModule::name( void ) const
{
  return std::string( "Aibs synapse Module" ); // Return name of the module
}

const std::string
allennest::AllenModule::commandstring( void ) const
{
  // Instruct the interpreter to load AllenModule-init.sli
  return std::string( "(allenmodule-init) run" );
}

//-------------------------------------------------------------------------------------

void
allennest::AllenModule::init( SLIInterpreter* i )
{
  // Register a synapse module
  nest::kernel()
    .model_manager.register_connection_model< AibsConnection< nest::
        TargetIdentifierPtrRport > >( "aibs_synapse" );
